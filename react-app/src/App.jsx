...
// --- 3. THE PROCEDURAL HUMANOID (WITH PANIC MODE) ---
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (distance < 0.2) {
      if (waypointIdx.current < WALK_PATH.length - 1) {
        ...
        if (typeof onDespawn === 'function') onDespawn(id); 
      }
    } else {
      // IF IN PANIC MODE, THEY SPRINT (2.5x speed)
...
export default function App() {
  ...
  // DIRECT ESP32 POLLING (Replaces Socket.io)
  useEffect(() => {
    const fetchSensorData = async () => {
      try {
        ...
        if (dist > 5 && dist < 80) {
          const now = Date.now();
          // Check if it's untripped AND if 500ms have passed since the last spawn
          if (!isTripped.current && (now - lastSpawnTime.current > SPAWN_COOLDOWN)) {
            isTripped.current = true; 
            lastSpawnTime.current = now; // Lock the cooldown timer
        ...
        setLogs(prev => [`[${new Date().toLocaleTimeString()}] HTTP_GET: ${JSON.stringify(newData)}`, ...prev].slice(0, 4));
