...
void dht_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (read_dht22_raw(&hum, &temp)) {
            real_temperature = temp - 2.5; // Calibration
            real_humidity = hum;
        }
...
void fast_sensor_task(void *pvParameters) {
    while(1) {
        ...
        // --- 2. ULTRASONIC MEDIAN FILTER ---
        float d1 = read_distance_cm(); vTaskDelay(pdMS_TO_TICKS(10));
        float d2 = read_distance_cm(); vTaskDelay(pdMS_TO_TICKS(10));
        float d3 = read_distance_cm(); 
        
        // Find the middle value (ignores sudden 2cm noise spikes!)
        float median = d1;
...
static esp_err_t data_get_handler(httpd_req_t *req) {
    ...
    // This bypasses the browser's strict security blocking!
...
void start_webserver(void) {
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        ESP_LOGI(TAG, "Web server started on /data!");
    }
}
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) esp_wifi_connect();
        ...
        ESP_LOGW(TAG, "=======================================");
        ESP_LOGW(TAG, "✅ CONNECTED! ENTER THIS IP IN REACT:");
        ESP_LOGW(TAG, "      http://" IPSTR "/data", IP2STR(&event->ip_info.ip));
        ESP_LOGW(TAG, "=======================================");
